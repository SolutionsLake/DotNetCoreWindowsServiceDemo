    ...
    internal class Program
    {
        private static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                var asService = !(Debugger.IsAttached || args.Any(x => x.Equals("--console")));

                if (asService)
                {
                    await builder.RunAsServiceAsync();      // .NET Core 3 uses UseWindowsService()
